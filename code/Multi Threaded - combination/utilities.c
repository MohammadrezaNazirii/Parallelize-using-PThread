...
void* ChunkToGrayscale(void* arg) {
    GrayScaleTask* task = (GrayScaleTask*) arg;

    for (size_t i = task->start; i < task->end; i += ADJACENT) {
        unsigned char red = task->grayValues[i];
        unsigned char green = task->grayValues[i + 1];
        unsigned char blue = task->grayValues[i + 2];

        unsigned char gray = 0.2126 * red + 0.7152 * green + 0.0722 * blue;
        task->grayValues[i] = gray;
        task->grayValues[i + 1] = gray;
        task->grayValues[i + 2] = gray;
    }
    pthread_exit(NULL);
}